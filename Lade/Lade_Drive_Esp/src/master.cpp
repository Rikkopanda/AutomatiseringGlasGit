...
bool modbusResultCb(Modbus::ResultCode event, uint16_t, void*) {
...
void setDriveEnable(bool en) {
  ...
  mb.writeHreg(DRIVE_ID, REG_PN070_COMM_STATE, val, modbusResultCb);
}
...
void loop() {
  ...
  if (setupStep < 3 && millis() - lastSetupTx > 200) {
    lastSetupTx = millis();
    switch (setupStep) {
      case 0: mb.writeHreg(DRIVE_ID, REG_PN068_COMM_EN, 0x0001, modbusResultCb); break; // SON via comms
      case 1: mb.writeHreg(DRIVE_ID, REG_PN168_SPD_SRC, 1, modbusResultCb); break;      // internal speed mode
  ...
  if (millis() - lastDrivePoll > 50) {
    ...
    mb.readHreg(DRIVE_ID, REG_DN08_ACTUAL_SPEED, (uint16_t*)&actualSpeed, 1, modbusResultCb);
    mb.readHreg(DRIVE_ID, REG_DN18_STATUS, &driveStatus, 1, modbusResultCb);
  }

  // poll the UI ESP32 every 150 ms — not time-critical
  if (millis() - lastUiPoll > 150) {
    lastUiPoll = millis();
    mb.readHreg(UI_ID, UI_REG_SETPOINT, &uiSetpoint, 1, modbusResultCb);
    mb.readHreg(UI_ID, UI_REG_RUN, &uiRun, 1, modbusResultCb);

    // act on what the UI asked for
    setDriveEnable(uiRun != 0);
    mb.writeHreg(DRIVE_ID, REG_PN169_SPEED_SET, uiSetpoint, modbusResultCb);

    // echo drive status back to the UI so it can display it
    mb.writeHreg(UI_ID, UI_REG_STATUS, driveStatus, modbusResultCb);
  }
